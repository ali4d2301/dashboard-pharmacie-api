...
from reporting import build_weekly_report, build_weekly_report_csv, get_default_report_recipients
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Envoie le rapport hebdomadaire des mouvements par e-mail.")
    ...
    parser.add_argument(
        "--recipients",
        help="Liste d'e-mails separes par des virgules. Par defaut: WEEKLY_REPORT_RECIPIENTS",
    )
    parser.add_argument("--dry-run", action="store_true", help="Genere le rapport sans l'envoyer")
    parser.add_argument(
        "--no-csv",
        action="store_true",
        help="N'ajoute pas la piece jointe CSV, meme si WEEKLY_REPORT_ATTACH_CSV est active",
    )
    ...
    recipients = (
        ...
        if args.recipients
        else get_default_report_recipients()
    )
    ...
    if settings.WEEKLY_REPORT_ATTACH_CSV and not args.no_csv:
